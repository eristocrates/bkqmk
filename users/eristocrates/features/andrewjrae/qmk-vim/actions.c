...
#endif // end ifdef QMK_VIM_TEXT_OBJECTS

// The actual change action
void change_action(void) {
    if (vim_emulation) {
        tap_code16(QMK_VIM_CHANGE);
    }
    insert_mode();
}
...
static bool process_replace_action(uint16_t keycode, const keyrecord_t *record) {
    if (record->event.pressed) {
        if (vim_emulation && keycode != KC_ESC) {
            tap_code(KC_DELETE);
            tap_code16(keycode);
            tap_code(KC_LEFT);
        }
...
#    ifndef QMK_VIM_REPEAT_BUF_SIZE
#        define QMK_VIM_REPEAT_BUF_SIZE 64
#    endif
...
static uint16_t repeat_buf[QMK_VIM_REPEAT_BUF_SIZE];
...
void add_repeat_keycode(uint16_t keycode) {
    if (repeat_state == RECORDING_REPEAT) {
        if (repeat_buf_idx < QMK_VIM_REPEAT_BUF_SIZE) {
...
void repeat_action(const keyrecord_t *record) {
    if (vim_emulation && repeat_state == VALID_REPEAT) {
