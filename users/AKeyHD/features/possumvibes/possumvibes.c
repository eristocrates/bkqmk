// https://github.com/possumvibes/qmk_userspace/tree/possumdev/users/possumvibes
...
bool        process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        // Layer Modes
        case NUMMODE:
            return num_mode_enable(record);
        case FUNMODE:
            return func_mode_enable(record);
        case NAVMODE:
            return nav_mode_enable(record);
        case SYMMODE:
            return sym_mode_enable(record);
        case MCRMODE:
            return macro_mode_enable(record);

        // Funky Symbol Shifts
        case KC_COMM:
            return override_shift(is_shifted, keycode, KC_EXLM, record);
        case KC_DOT:
            return override_shift(is_shifted, keycode, KC_QUES, record);
        case KC_DQUO:
            return send_autopair_on_shift(is_shifted, KC_DQUO, KC_DQUO, keycode, record);
        case KC_GRV:
            return send_autopair_on_shift(is_shifted, KC_GRV, KC_GRV, keycode, record);

        case KC_LPRN:
            return send_autopair_on_shift(is_shifted, KC_LPRN, KC_RPRN, keycode, record);
        case KC_RPRN:
            return send_string_c_function(is_shifted, keycode, record);
        case ANGLEBR:
            return send_autopair(KC_LABK, KC_RABK, record);
        case BRCKETS:
            return is_shifted ? send_autopair(KC_LCBR, KC_RCBR, record) : send_autopair(KC_LBRC, KC_RBRC, record);
        ...
        case MD_LINK:
            return send_string_markdown_link(record);
        ...
        case KY_V1:
            return send_string_version(is_shifted, KC_1, record);
        case KY_V2:
            return send_string_version(is_shifted, KC_2, record);
        case KY_V3:
            return send_string_version(is_shifted, KC_3, record);
        ...
        case VI_AW:
            return send_string_vi_yiw(is_shifted, false, KC_A, true, record);
        case VI_IW:
            return send_string_vi_yiw(is_shifted, false, KC_I, true, record);
        case VI_YAW:
            return send_string_vi_yiw(is_shifted, true, KC_A, true, record);
        case VI_YIW:
            return send_string_vi_yiw(is_shifted, true, KC_I, true, record);
